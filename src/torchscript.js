        ...
        if (expression.type == 'call' && (target.type == 'id' || target.type == 'tuple')) {
            ...
            if (name.startsWith(namespace)) {
                ...
                while (args.length > 0) {
                    ...
                    if (argument.type == 'id' && this._state[argument.value]) {
    ...
    _attributeExpression(expression) {
        if (expression.type == 'id') {
            if (this._state[expression.value]) {
                return { type: 'number', value: this._state[expression.value] };
            }
        }
    ...
    _attributeStatement(statement) {
        if (statement.type == '=' &&
            ...
            if (statement.expression.type == 'call' &&
                ...
                statement.expression.arguments.length == 1) {
                this._state[statement.target.value] = statement.expression.arguments[0];
                return true;
            }
            // _0 = torch.size(... , ...)
            if (statement.expression.type == 'call' &&
